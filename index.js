...
function renderHero(startTestCallback) {
...
function renderArticleDetail(container, index) {
...
function renderTestCard(title, description, renderContentFn, id, isCompleted, isActive) {
...
function renderFocusTest(parent, onComplete) {
    let isTestRunning = false;
    let hits = 0;
    let clicks = 0;
    let timeRemaining = FOCUS_TEST_DURATION_MS / 1000;
    let dotX = 50, dotY = 50;
    let countdownInterval, dotMoveInterval;
    let finalScore = null;
...
function renderAfterimageTest(parent, onComplete) {
    ...
    if (testResults.afterimage !== null) {
        ...
        const completedPlaceholder = Array.from(displayArea.children).find(el => el.classList.contains('afterimage-placeholder-completed'));
...
function renderColorTest(parent, onComplete) {
    let currentRound = 0;
    let score = 0;
    let differentColor;
...
function renderFlickerTest(parent, onComplete) {
    let flickerHz = 5;
    ...
    slider.oninput = (e) => {
        if (isFlickering) {
            flickerHz = parseInt(e.target.value);
...
function renderContrastTest(parent, onComplete) {
    let currentRound = 0;
    let score = 0;
    let currentNumber, currentOpacity;
    let testInProgress = false;
    ...
    function drawOnCanvas(number, opacity) {
    ...
    drawOnCanvas(undefined, undefined);
...
function renderEyestrainTest(parent, onUpdate) {
    const symptoms = {
        dryness: { label: "Dryness", inputEl: null, valueEl: null },
        blur: { label: "Blurriness", inputEl: null, valueEl: null },
        irritation: { label: "Irritation", inputEl: null, valueEl: null },
    };
    ...
    submitButton.onclick = () => {
        ...
        submitButton.disabled = true;
        completeTest('eyestrain', totalScore);
    };

    if (testResults.eyestrain !== null) {
        submitButton.disabled = true;
        Object.keys(symptoms).forEach(key => {
            if (symptoms[key].inputEl) {
                symptoms[key].inputEl.disabled = true;
            }
        });
...
function renderFinalAssessment(results) {
...
function renderApp() {
    ...
    if (currentView === 'home') {
        ...
        testsContainer.appendChild(renderTestCard('6. Eye Strain Symptoms', 'Rate your current level of eye discomfort.', (el) => renderEyestrainTest(el, (symptom, value) => { testResults[symptom] = value; }), 'eyestrain-test-card', testResults.eyestrain !== null, currentTestIndex === 5));
        mainContent.appendChild(testsContainer);

        if (TEST_ORDER.every(key => testResults[key] !== null)) {
            mainContent.appendChild(renderAdPlaceholder());
